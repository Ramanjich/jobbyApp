...
import SimilarJodDetails from '../SimilarJobs'
import './index.css'
...
class JobItemDetails extends Component {
  ...
  getDetailsOfJob = async () => {
    ...
    const {id} = params
    console.log(id)
    const accessTokenD = Cookies.get('jwt_token')
    ...
    const responseOfJobsDetails = await fetch(detailsUrl, optionsData)

    if (responseOfJobsDetails.ok === true) {
      const fetchedDetailsData = await responseOfJobsDetails.json()
      const updatedJobDetailsData = [fetchedDetailsData.job_details].map(
        eachItem => ({
      ...
      const updatedSimilarJobDetails = fetchedDetailsData.similar_jobs.map(
        eachItem => ({
          companyLogoUrl: eachItem.company_logo_url,
          id: eachItem.id,
          jobDescription: eachItem.job_description,
      ...
      this.setState({
        ...
        relatedJobs: updatedSimilarJobDetails,
  ...
  renderJobDetailsSuccessView = () => {
    const {jobDetailsData, relatedJobs} = this.state
